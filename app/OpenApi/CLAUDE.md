...
Domain-specific response shapes. Per the Phase 8 convention, `#[OA\Schema]` belongs on the Data class that owns the data:
...
`Common/` is empty. The last occupant was `ErrorResponseSchema` — its annotation was moved to `app/Shared/Data/ApiErrorData` in Phase 8 (see `docs/agentic/BUILD.md`).
