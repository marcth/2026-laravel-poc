...
- **Never commit directly to `develop`:** If the current branch is `develop`, always create a new branch first (`git checkout -b <branch-name>`), commit there, push, and open a PR targeting `develop`. Never run `git commit` or `git push` while on `develop`.
